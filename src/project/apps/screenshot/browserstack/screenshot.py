    ...
    def __init__(self, **kwargs):
        ...
        if kwargs.get('auth'):
            if not len(kwargs.get('auth')) == 2:
                raise exceptions.ImproperlyConfiguredException("Auth is a tuple of \
                    ('username', 'token')")
            self.auth = kwargs.get('auth')
        else:
            raise exceptions.ImproperlyConfiguredException("Please define an \
                    auth=('username', 'token') tuple in object constructor")

    def _process_response(self, req):
        if req.status_code == 401:
            raise exceptions.AuthenticationError("Authentication Failure")
        elif req.status_code == 403:
            raise exceptions.ScreenshotNotAllowedError("Screenshot not allowed")
        elif req.status_code == 422:
            raise exceptions.InvalidRequestError(json.loads(req.content))
        elif req.status_code == 200:
            return req
        else:
            raise exceptions.UnexpectedError(req.status_code)
